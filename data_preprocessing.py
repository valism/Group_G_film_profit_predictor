...
def genre_enc(imdb_df):
    ...
    genre_counts = genre_counts.merge(pd.DataFrame(genres_df['genre_2'].value_counts()), how='outer', left_index=True,
                                      right_index=True)
    genre_counts = genre_counts.merge(pd.DataFrame(genres_df['genre_3'].value_counts()), how='outer', left_index=True,
                                      right_index=True)
    genre_counts['Count'] = genre_counts.sum(axis=1)
    genre_counts["Percentage"] = (genre_counts["Count"] / genre_counts["Count"].sum() * 100).round(1)
    print(
        "The table below shows the occurence of each genre in the imdb database and the total percentage of films they occur in")
    print(
        "95.7% of films are covered by first 14 genres. the final 6 genres are only found in 4.3 percent of films, so are aggregated to form an 'Other' genre grouping")
...
# film_pair_selection selects as close to 10,000 pairs as possible that ensures equal number of films across each of
...
def film_overlapping_genre_selection(imdb_df):
    ...
    for col in cols:
        ...
        genre_mtx = pd.DataFrame(np.diag(genre_sel["duration"]), columns=genre_sel["imdb_title_id"],
                                 index=genre_sel["imdb_title_id"])
